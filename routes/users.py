...
async def delete_user(id: int, db: Session = Depends(get_db)):
    db_user = crud.users.get_user(db=db, id=id)
    if db_user is None:
        raise HTTPException(status_code=404, detail="Userario no exite, no se puede eliminar")
    crud.users.delete_user(db=db, id=id)
    return db_user
...
async def update_user(id: int, user: schemas.users.UserUpdate, db: Session = Depends(get_db)):
    db_user = crud.users.get_user(db=db, id=id, user=user)
    if db_user is None:
        raise HTTPException(status_code=404, detail="User no exite, no actulizado")
    updated_user = crud.users.update_user(db=db, id=id, user=user)
    return updated_user
